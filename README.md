...
<h3>手順</h3>
<h4>手順1.AがGitHub上にリモートリポジトリを用意し、index.html（"Hello"と記述）をmainブランチにPushする</h4>
<p>ここでは、ターミナルでリモートリポジトリを作成し、GitとGitHubの連携の確認を行った。</p>
...
<h4>手順2.Bがリポジトリをcloneし、作業ブランチを作成。index.htmlを編集してPushし、Aへプルリクエストを出す。</h4>
...
```
<p>この下の画像がOpen a pull request画面です。</p>
...
<h4>手順3. AがBのプルリクエストをレビューし、mainブランチにマージする。</h4>
...
<p>下の画像がマージの処理が正常に終了し、mainブランチに統合した状態です。</p>
...
<h4>手順4. Aがローカルのmainブランチを最新化（pull）し、作業ブランチを作成。index.htmlを編集してPRを作成・マージする。</h4>
<p>次にAのユーザーが、ターミナル上で、mainブランチを最新化した後、新しいブランチを作成します。</p>
<p>新しいブランチの中で、viコマンドを使用し、index.htmlを編集していき、A自身でプルリクエストとマージを行う。</p>
<p>まずは、mainブランチの最新化と作業ブランチの作成を行います。</p>

```bash
$ git pull origin main
$ git switch -c tejun4
```
<p>次にviコマンドによるindex.htmlの編集を行います。</p>
...
<h4>手順5. Bがローカルのmainブランチを最新化（pull）し、作業ブランチを作成。stylesheet.cssを追加してAへプルリクエストを出す。</h4>
<p>手順4.でAが行っていたmainブランチの最新化をBのユーザーが行い、その後に新しい作業ブランチを作成する。</p>
...
```
<p>その後、アップロードしたstylesheet.cssのプルリクエストを作成します。</p>
...
<h4>手順6. AがBのプルリクエストをレビューし、mainブランチにマージする。</h4>
